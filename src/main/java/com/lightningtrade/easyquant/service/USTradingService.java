...
public class USTradingService extends AbstractTradingService {
    ...
    protected boolean isEnabled() {
        return tradingConfig.getUs().isEnabled();
    }

    @Override
    protected List<TradingConfig.Symbol> getSymbols() {
        return tradingConfig.getUs().getSymbols();
    }

    @Override
    protected TradingConfig.Strategy getStrategyConfig() {
        return tradingConfig.getUs().getStrategy();
    }
        ...
        switch (signal) {
            case BUY:
                if (position <= 0) {
                    ...
                    int lotSize = tradingConfig.getUs().getSymbols().stream()
